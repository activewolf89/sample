...
import AddOntoSearchSegment from 'containers/Shop/Helpers/AddOntoSearchSegment.js';
import FilterProductBySearch from 'containers/Shop/Helpers/FilterProductBySearch.js';
import Span from 'shared/Span';
import DestroySearchSegment from 'containers/Shop/Helpers/DestroySearchSegment.js';
...
export default class Ranges extends React.Component {
  constructor(props) {
    ...
    this.state = {
      whatIfValue: this.props.numberForReveal,
  ...
  handleAfterChange = function(){
    if(this.state.triggeredLine !== (this.state.lowerBound + '-' + this.state.upperBound)){
      this.props.history.push(this.props.rootPath + '?' + this.props.rangeKey + '=' + this.state.lowerBound + '-' + this.state.upperBound)
      this.props.travelTo()
      if(this.props.onHamburgerClick){
        this.props.onHamburgerClick('close')
      }
      this.setState({
    ...
    var upperValue = this.state.value[1];
      ...
      if(upperValue !== high){
        ...
        // upperValue = (Math.round(this.state.initialUpperBound - (this.state.initialUpperBound * (1-(high/100))) + this.state.initialLowerBound) * 10) / 10
          ...
          var erasedEntirePathnameSearch = DestroySearchSegment(this.props.history,this.props.DetailField, true,true)
          var updatedSearchString = AddOntoSearchSegment(this.props.DetailField,`${this.state.lowerBound}-${upperValue}`,this.props.history,true,erasedEntirePathnameSearch)
        this.setState({
          value: value,
          upperBound: upperValue,
          whatIfValue: FilterProductBySearch(this.props.allProducts,updatedSearchString,this.props.catId).length
...
// /engagement-rings/shop?/engagement-rings/shoponSale=true&?ItemRetailPrice=2517.7-77000
        ...
        //modify lower value
          ...
          // console.log(this.props.history.location.search.indexOf([this.props.DetailField]))
           erasedEntirePathnameSearch = DestroySearchSegment(this.props.history,this.props.DetailField, true,true)
           updatedSearchString = AddOntoSearchSegment(this.props.DetailField,`${lowerValue}-${this.state.upperBound}`,this.props.history,true,erasedEntirePathnameSearch)
  ...
  render() {
    return (
      <Div  padding='20px '>
        ...
        {
          false ?
            <Div display="flex" flexWrap="nowrap" justifyContent="space-between"  padding="10px 0" minWidth="300px">
            ...
            <Div display="flex" flexWrap="nowrap" justifyContent="space-between"  padding="10px 0" minWidth='200px'>
              <Span style={{border:'1px solid black'}} id='min' backgroundColor='white' color='gray' padding='5px'>
                {
                  this.state.lowerBound
                }
              </Span>
              <Span style={{border:'1px solid black'}} id='max' backgroundColor='white' color='gray' padding='5px'>
                {
